...
public class FileFilter_ApachePOI extends JFrame {
	...
	public static void main(String[] args) {
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					FileFilter_ApachePOI frame = new FileFilter_ApachePOI();
	...
	class DetectThread extends Thread {
		...
		public void run() {
			...
			if(files.size() >= 1) {
				...
				File filedir = new File(filePath + "\\apache poi");
				filedir.mkdir();
			}
			...
			for(File file : files) {
				try {
					...
					if(ReadWriteFile.canBeOpen(file)) {
						
						listModel.addElement(file.getName());
						list_smellFiles.setModel(listModel);
						
						String des = filePath + "\\apache poi\\" + file.getName();
